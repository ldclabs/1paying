<script lang="ts">
  ...
  const sizeClasses = {
    ...
    lg: 'sm:max-w-2xl',
    xl: 'sm:max-w-4xl',
    full: 'sm:max-w-5xl lg:max-w-6xl xl:max-w-7xl'
  } as const
...
>
  {#each modalStore as modal, index (modal.id)}
    <div
      class="pointer-events-auto relative flex h-full w-full items-start justify-center overflow-y-auto px-4 pb-4 sm:px-8 sm:pb-8"
      style={`z-index: ${index + 1}; padding-top: clamp(1.5rem, 38vh, 12rem);`}
    >
      <div
      ...
      ></div>
      <div
        class={`relative z-10 flex max-h-[calc(100vh-4rem)] w-full flex-col gap-6 overflow-y-auto rounded-xl bg-white/95 p-6 text-slate-900 shadow-xl ring-1 ring-black/5 focus:outline-none dark:bg-slate-900/95 dark:text-slate-100 ${sizeClasses[modal.size ?? 'md']}`}
        role="dialog"
        aria-modal="true"
